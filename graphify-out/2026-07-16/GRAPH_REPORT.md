...
## Corpus Check
- 282 files · ~175,016 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 958 nodes · 1010 edges · 82 communities (77 shown, 5 thin omitted)
...
3. `Web SDK` - 10 edges
4. `Firebase AI Logic Basics` - 9 edges
5. `Configuration Reference` - 9 edges
6. `iOS SDK` - 9 edges
7. `Templates` - 9 edges
8. `Firebase Authentication on Android (Kotlin)` - 8 edges
9. `Firebase SQL Connect` - 8 edges
10. `Native SQL Examples` - 8 edges
...
- `PortfolioMon Entry HTML` --conceptually_related_to--> `React + Vite Template README`  [INFERRED]
  index.html → README.md
- `App()` --calls--> `useAudio()`  [EXTRACTED]
  src/App.jsx → src/hooks/useAudio.js
- `App()` --calls--> `useBackgroundMusic()`  [EXTRACTED]
  src/App.jsx → src/hooks/useBackgroundMusic.js
...
Cohesion: 0.09
Nodes (26): App(), AudioControls(), BossSelect(), ContentScreen(), MainMenu(), NameEntry(), StartScreen(), audioUrl() (+18 more)
...
Cohesion: 0.15
Nodes (18): BattleScreen(), unlockedSectionLabel(), ResultOverlay(), TypeChart(), BOSS_ASSETS, HOLLOW_DAMAGE, MOVE_DESCRIPTIONS, movesFor() (+10 more)
...
Cohesion: 0.25
Nodes (8): 1. Generate Firestore Rules, Critical Constraints, Critical Directives for Secure Generation, **CRITICAL** RBAC Guidelines, Mandatory: User Data Separation (The "No Mixed Content" Rule), Phase-1: Codebase Analysis, Phase-2: Security Rules Generation, Workflow
...
### Community 57 - "Firebase AI Logic on Android (Kotlin)"
Cohesion: 0.50
...
## Knowledge Gaps
- **574 isolated node(s):** `PackageDescription`, `Foundation`, `PathKit`, `name`, `private` (+569 more)
